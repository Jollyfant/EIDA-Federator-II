...
function validateDataselectRequest(request) {
  ...
  validateSupported(request, ALLOWED_PARAMETERS);
...
function validateSupported(request, allowed) {

  /* FUNCTION validateSupported
   * Checks if keys passed are supported by the service 
   */
...
function validateStationRequest(request) {
  ...
  if(request.query.format) {
    if(!FORMAT_ENUM.includes(request.query.format)) {
      throw new Error("The requested format: " + request.query.format + " is invalid. Expected one of: " + FORMAT_ENUM.join(", ") + ".");
    }
  }
